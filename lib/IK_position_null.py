...
class IK:
    ...
    def __init__(self,linear_tol=1e-3, angular_tol=1e-2, max_steps=1000, min_step_size=1e-5):
        """
    ...
    def displacement_and_axis(target, current):
        ...
        displacement = (target[:3,3] - current[:3,3]).reshape(3,)
    ...
    def distance_and_angle(G, H):
        ...
        angle = acos(min(max(((np.trace(np.matmul(R_G.T, R_H)) - 1) / 2), -1), 1))
    ...
    def is_valid_solution(self,q,target):
        ...
        if distance < self.linear_tol and angle < self.angular_tol and all(self.lower<q) and all(self.upper>q):
            success = True
            message = "Solution found successfully!"

        elif any(self.lower>q) or any(self.upper<q):
            success = False
            message = "Out of Joint Limits!"

        elif distance > self.linear_tol or angle > self.angular_tol:
            success = False
    ...
    def end_effector_task(q,target,method):
        ...
        displacement, axis = IK.displacement_and_axis(target, fk.forward(q)[-1])
        J = calcJacobian(q)
        difference = np.concatenate((displacement, axis))

        if method == 'J_trans':
            dq = np.matmul(J.T,difference)
        elif method == 'J_pseudo':
            J_pinv = np.linalg.pinv(J)
            dq = np.matmul(J_pinv,difference)
        else:
    ...
    def inverse(self, target, seed, method='J_pseudo', alpha=0.1, lamda=5):
        """
        ...
        while True:
            ...
            null = np.matmul(np.eye(7) - np.matmul(np.linalg.pinv(J),J),dq_center).reshape(7,)
...
if __name__ == "__main__":
    ...
    np.array([ pi/2,    0,     0, -pi/2,     0, pi/2, pi/4 ]),
    np.array([ pi/2,    -pi/2,     0, -pi/2,     0, pi/2, pi/4 ]),
    ...
    for seed in configurations:

        # Using pseudo-inverse 
        q_pseudo, rollout_pseudo, success_pseudo, message_pseudo = ik.inverse(target, seed, method='J_pseudo', alpha=0.1, lamda=5)
        ...
        # Using pseudo-inverse 
        q_trans, rollout_trans, success_trans, message_trans = ik.inverse(target, seed, method='J_trans', alpha=0.5, lamda=0.5)
