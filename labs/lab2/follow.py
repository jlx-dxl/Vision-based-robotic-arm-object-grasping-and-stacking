...
class JacobianDemo():
    ...
    fk = FK()
    point_pub = rospy.Publisher('/vis/trace', geometry_msgs.msg.PointStamped, queue_size=200)
    counter = 0
    ...
    def eight(t,fx=1,fy=2,rx=.15,ry=.1):
        ...
        # Lissajous Curve: which means fy = 2 * fx
        x0 = np.array([0.307, 0, 0.487]) # corresponds to neutral position
        xdes = x0 + np.array([rx*sin(fx*t),ry*sin(fy*t),0])
        vdes = np.array([rx*fx*cos(fx*t),ry*fy*cos(fy*t),0])   # v = x'
        return xdes, vdes

    def ellipse(t,f=1,rx=0.10,ry=0.20):
        """
        ...
        x0 = np.array([0.307 + rx, 0, 0.487]) # corresponds to neutral position
